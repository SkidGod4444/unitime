...
// Get all attendance sessions for history display
attendance.get("/all-history/:userId", async (c) => {
  try {
    const userId = c.req.param("userId");

    // We fetch sessions across the user's organization if applicable, or matching their enrollments natively. 
    // For a representative or admin they may see org-level sessions.
    // For standard users, we might just query the sessions of courses they are enrolled in.
    
    // As a simple robust fallback mirroring the history api logic:
    ...
    // Fetch all sessions related to courses within this organization (or created by this user as fallback)
    const sessions = await prisma.attendanceQRSession.findMany({
      where: {
        OR: [
          { createdBy: userId },
