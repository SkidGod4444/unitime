  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-zinc-900">
      ...
      <View className="flex-row items-center px-6 pt-2 pb-4">
        <TouchableOpacity onPress={() => router.back()} className="p-3 bg-gray-100 dark:bg-gray-800 rounded-full active:opacity-70 mr-4">
          <Ionicons name="arrow-back-outline" size={24} color="#374151" />
        </TouchableOpacity>
        <View className="flex-1">
          <Text className="text-sm font-medium text-gray-500 uppercase tracking-wider">Manage</Text>
          <Text className="text-3xl font-bold text-gray-900 dark:text-white">Lab Groups</Text>
        </View>
      </View>
      ...
      {canManage && (
        <View className="mx-4 mb-3 bg-white rounded-2xl p-4 border border-gray-100">
          <Text className="text-sm font-semibold text-gray-700 mb-2">Create Group</Text>
      ...
      <Modal
        ...
        animationType="slide"
        onRequestClose={() => setMembersModal({ groupId: null, visible: false })}
      >
        <SafeAreaView className="flex-1 bg-gray-50">
          <View className="flex-row items-center px-4 py-3 border-b border-gray-100 bg-white">
            <TouchableOpacity onPress={() => setMembersModal({ groupId: null, visible: false })} className="p-2 mr-2">
              <Ionicons name="close" size={22} color="#6b7280" />
            </TouchableOpacity>
            <Text className="text-base font-bold text-gray-900">Group Members</Text>
          </View>
          <FlatList
            data={membersModal.groupId ? membersByGroup[membersModal.groupId] || [] : []}
            keyExtractor={(m) => m.id}
            contentContainerStyle={{ padding: 16 }}
            renderItem={({ item }) => (
              <View className="bg-white rounded-2xl p-4 mb-3 border border-gray-100">
                <Text className="text-base font-semibold text-gray-900">{item.name}</Text>
                <Text className="text-xs text-gray-500 mt-0.5">{item.studentProfile?.admissionNumber || item.email}</Text>
              </View>
            )}
          />
        </SafeAreaView>
