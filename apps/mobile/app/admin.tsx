...
type ClassItem = { id: string; name: string; section: string; strength: number; year: number };
type Professor = { id: string; name: string; subject: string; email: string };
type Course    = { id: string; code: string; name: string; credits: number; professor: string };
type Attendance = { id: string; course: string; className: string; date: string; present: number; total: number };
...
const USERS: User[] = [
  ...
  { id: 'u3', name: 'Charlie Brown',   email: 'charlie@uni.edu', role: 'REPRESENTATIVE'  },
  { id: 'u4', name: 'Diana Prince',    email: 'diana@uni.edu',   role: 'STUDENT'         },
  { id: 'u5', name: 'Evan Davis',      email: 'evan@uni.edu',    role: 'STUDENT'         },
  { id: 'u6', name: 'Fiona Gallagher', email: 'fiona@uni.edu',   role: 'PROFESSOR'       },
];
...
const PROFESSORS: Professor[] = [
  { id: 'p1', name: 'Dr. Bob Johnson',     subject: 'Data Structures',   email: 'bob@uni.edu'   },
  { id: 'p2', name: 'Dr. Fiona Gallagher', subject: 'Computer Networks', email: 'fiona@uni.edu' },
  { id: 'p3', name: 'Dr. Mark Spencer',    subject: 'Operating Systems', email: 'mark@uni.edu'  },
  { id: 'p4', name: 'Dr. Sarah Connor',    subject: 'DBMS',              email: 'sarah@uni.edu' },
];

const COURSES: Course[] = [
  { id: 'cr1', code: 'CS201', name: 'Data Structures',   credits: 4, professor: 'Dr. Bob Johnson'     },
  { id: 'cr2', code: 'CS301', name: 'Operating Systems', credits: 3, professor: 'Dr. Mark Spencer'    },
  { id: 'cr3', code: 'CS401', name: 'Computer Networks', credits: 3, professor: 'Dr. Fiona Gallagher' },
  { id: 'cr4', code: 'CS501', name: 'DBMS',              credits: 4, professor: 'Dr. Sarah Connor'    },
];

const ATTENDANCES: Attendance[] = [
  { id: 'a1', course: 'CS201 – Data Structures',   className: 'CSE-A', date: 'Feb 21, 2026', present: 55, total: 62 },
  { id: 'a2', course: 'CS301 – Operating Systems', className: 'CSE-B', date: 'Feb 21, 2026', present: 48, total: 58 },
  { id: 'a3', course: 'CS401 – Computer Networks', className: 'IT-A',  date: 'Feb 20, 2026', present: 53, total: 55 },
  { id: 'a4', course: 'CS501 – DBMS',              className: 'ECE-A', date: 'Feb 19, 2026', present: 42, total: 60 },
];
...
function RolesTab({ onAddUserPress }: { onAddUserPress: () => void }) {
  ...
  const handleDelete = (user: User) => {
    Alert.alert('Remove User', `Remove ${user.name} from the system?`, [
      ...
      { text: 'Remove', style: 'destructive', onPress: () => setUsers((p) => p.filter((u) => u.id !== user.id)) },
    ]);
  };
  ...
  return (
    <>
            ...
            <View className="bg-white rounded-2xl p-4 mb-3 border border-gray-100 shadow-sm">
              <View className="flex-row items-center justify-between">
                <View className="flex-1">
                  ...
                  <View className={`self-start mt-2 px-2.5 py-0.5 rounded-full ${colors.bg}`}>
                    <Text className={`text-xs font-bold ${colors.text}`}>{user.role}</Text>
                  </View>
                </View>
                <RowActions onEdit={() => setRoleModal(user)} onDelete={() => handleDelete(user)} />
              </View>
            </View>
          );
        }}
      ...
      <Modal visible={!!roleModal} transparent animationType="fade" onRequestClose={() => setRoleModal(null)}>
        <Pressable className="flex-1 bg-black/40 justify-end" onPress={() => setRoleModal(null)}>
          <View className="bg-white rounded-t-3xl p-6">
            <Text className="text-lg font-bold text-gray-900 mb-1">Change Role</Text>
            <Text className="text-sm text-gray-500 mb-4">{roleModal?.name}</Text>
            {ALL_ROLES.map((role) => {
              ...
              return (
                <Pressable
                  ...
                  className={`flex-row items-center justify-between px-4 py-3 rounded-xl mb-2 border ${
                    active ? 'border-indigo-300 bg-indigo-50' : 'border-gray-100 bg-gray-50'
                  }`}
                >
                  <View className={`px-2.5 py-0.5 rounded-full ${colors.bg}`}>
                    <Text className={`text-xs font-bold ${colors.text}`}>{role}</Text>
                  </View>
                  {active && <Ionicons name="checkmark-circle" size={20} color="#4f46e5" />}
                </Pressable>
              );
            })}
          </View>
...
function AddUserModal({ visible, onClose, onAdd }: { visible: boolean; onClose: () => void; onAdd: (user: User) => void }) {
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView className="flex-1 bg-gray-50">
        ...
        <View className="flex-row items-center justify-between px-4 py-3 bg-white border-b border-gray-100">
          ...
          >
            <Text className={`font-bold text-sm ${ selected ? 'text-white' : 'text-gray-400' }`}>Save</Text>
          </TouchableOpacity>
        </View>
        ...
        <ScrollView className="flex-1 px-4 pt-4" keyboardShouldPersistTaps="handled">
            ...
            return (
              ...
              >
                <View className="flex-row items-center gap-x-3 flex-1">
                  {/* Avatar */}
                  <View className={`w-9 h-9 rounded-full items-center justify-center ${ isSelected ? 'bg-indigo-200' : 'bg-gray-100' }`}>
                    <Text className={`font-bold text-base ${ isSelected ? 'text-indigo-700' : 'text-gray-500' }`}>
                      {user.name[0]}
                    </Text>
                  </View>
                  <View className="flex-1">
                    <Text className={`font-semibold ${ isSelected ? 'text-indigo-800' : 'text-gray-800' }`}>{user.name}</Text>
                    <Text className="text-xs text-gray-500 mt-0.5">{user.email}</Text>
                  </View>
                </View>
                {isSelected
          ...
          {selected && (
            <View className="mt-4 mb-6">
              <Text className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-2">Assign Role</Text>
              {ALL_ROLES.map((role) => {
                const colors = ROLE_COLORS[role];
                ...
                return (
                  <Pressable
                    ...
                    className={`flex-row items-center justify-between px-4 py-3.5 rounded-xl mb-2 border ${
                      active ? 'border-indigo-300 bg-indigo-50' : 'border-gray-100 bg-white'
                    }`}
                  >
...
function ClassesTab() {
  ...
  const handleDelete = (cls: ClassItem) => {
    Alert.alert('Delete Class', `Delete ${cls.name} – Sec ${cls.section}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => setClasses((p) => p.filter((c) => c.id !== cls.id)) },
    ]);
  };
    ...
    <FlatList
      ...
      renderItem={({ item: cls }) => (
        <View className="bg-white rounded-2xl p-4 mb-3 border border-gray-100 shadow-sm">
          <View className="flex-row items-center justify-between">
            <View className="flex-1">
              <Text className="text-base font-bold text-gray-900">{cls.name} – Sec {cls.section}</Text>
              <View className="flex-row gap-x-3 mt-1.5">
                <View className="flex-row items-center gap-x-1">
                  <Ionicons name="people-outline" size={13} color="#6b7280" />
                  <Text className="text-xs text-gray-500">{cls.strength} students</Text>
                </View>
                <View className="flex-row items-center gap-x-1">
            ...
            <RowActions
              onEdit={() => Alert.alert('Edit Class', `Editing ${cls.name} – Sec ${cls.section}`)}
...
function ProfessorsTab() {
  ...
  const handleDelete = (p: Professor) => {
    Alert.alert('Remove Professor', `Remove ${p.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Remove', style: 'destructive', onPress: () => setProfs((prev) => prev.filter((x) => x.id !== p.id)) },
    ]);
  };
          ...
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center gap-x-3 flex-1">
              <View className="w-10 h-10 rounded-full bg-indigo-100 items-center justify-center">
                <Text className="text-indigo-600 font-bold text-base">{prof.name.charAt(prof.name.indexOf(' ') + 1)}</Text>
              </View>
              <View className="flex-1">
                <Text className="text-base font-bold text-gray-900">{prof.name}</Text>
                <Text className="text-xs text-gray-500 mt-0.5">{prof.subject}</Text>
                <Text className="text-xs text-indigo-500 mt-0.5">{prof.email}</Text>
              </View>
            </View>
            <RowActions
              onEdit={() => Alert.alert('Edit Professor', `Editing ${prof.name}`)}
...
function CoursesTab() {
  ...
  const handleDelete = (c: Course) => {
    Alert.alert('Delete Course', `Delete ${c.code} – ${c.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => setCourses((p) => p.filter((x) => x.id !== c.id)) },
    ]);
  };
            ...
            <View className="flex-1">
              <View className="flex-row items-center gap-x-2">
                <View className="bg-indigo-600 px-2 py-0.5 rounded-md">
                  <Text className="text-white text-xs font-bold">{course.code}</Text>
                </View>
                <Text className="text-base font-bold text-gray-900 flex-1" numberOfLines={1}>{course.name}</Text>
              </View>
              <View className="flex-row gap-x-3 mt-1.5">
                <View className="flex-row items-center gap-x-1">
                  <Ionicons name="star-outline" size={13} color="#6b7280" />
                  <Text className="text-xs text-gray-500">{course.credits} credits</Text>
                </View>
                <View className="flex-row items-center gap-x-1">
                  <Ionicons name="person-outline" size={13} color="#6b7280" />
                  <Text className="text-xs text-gray-500">{course.professor}</Text>
                </View>
              </View>
            </View>
            <RowActions
              onEdit={() => Alert.alert('Edit Course', `Editing ${course.code} – ${course.name}`)}
        ...
        return (
          <View className="bg-white rounded-2xl p-4 mb-3 border border-gray-100 shadow-sm">
            <View className="flex-row items-start justify-between">
              <View className="flex-1">
                <Text className="text-base font-bold text-gray-900" numberOfLines={1}>{att.course}</Text>
                <View className="flex-row gap-x-3 mt-1">
                  <View className="flex-row items-center gap-x-1">
                    <Ionicons name="school-outline" size={13} color="#6b7280" />
                    <Text className="text-xs text-gray-500">{att.className}</Text>
                  </View>
                  <View className="flex-row items-center gap-x-1">
              ...
              <View className={`px-2.5 py-1 rounded-full ml-2 ${isLow ? 'bg-red-100' : 'bg-green-100'}`}>
                <Text className={`text-xs font-bold ${isLow ? 'text-red-700' : 'text-green-700'}`}>{pct}%</Text>
              </View>
            </View>
            <View className="mt-3">
              ...
              <View className="flex-row justify-between mt-1">
                <Text className="text-xs text-gray-400">{att.present} present</Text>
                <Text className="text-xs text-gray-400">{att.total - att.present} absent</Text>
              </View>
            </View>
          </View>
        );
...
export default function AdminPage() {
  ...
  const STAT_CARDS = [
    { label: 'Users',       value: USERS.length,       icon: 'people-outline',     color: '#4f46e5' },
    { label: 'Classes',     value: CLASSES.length,     icon: 'school-outline',     color: '#0891b2' },
    { label: 'Professors',  value: PROFESSORS.length,  icon: 'person-outline',     color: '#7c3aed' },
    { label: 'Courses',     value: COURSES.length,     icon: 'book-outline',       color: '#059669' },
  ];
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View className="px-4 pt-4 mb-4">
          ...
          <Text className="text-sm text-gray-500 mt-1">Manage platform entities and user permissions.</Text>
        </View>
            ...
            {STAT_CARDS.map((s) => (
              ...
              >
                <View className="flex-row items-center gap-x-2.5">
                  <View className="w-9 h-9 rounded-xl items-center justify-center" style={{ backgroundColor: s.color + '18' }}>
                    <Ionicons name={s.icon as any} size={18} color={s.color} />
                  </View>
                  <View>
                    <Text className="text-2xl font-extrabold text-gray-900">{s.value}</Text>
        ...
        {/* Active Tab Content */}
        <View className="px-4">
          {activeTab === 'roles'      && <RolesTab onAddUserPress={() => setAddUserVisible(true)} />}
