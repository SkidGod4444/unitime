...
export default function QuickActionsListener() {
  ...
  useQuickActionCallback(async (action) => {
    if (action.id === "clear_cache" || action?.params?.action === "clear_cache") {
      try {
        const lastClearedStr = await AsyncStorage.getItem("last_clear_cache_time");
        if (lastClearedStr) {
          ...
          if (now - lastCleared < TWENTY_FOUR_HOURS) {
            const hoursLeft = Math.ceil((TWENTY_FOUR_HOURS - (now - lastCleared)) / (60 * 60 * 1000));
            Alert.alert(
              "Rate Limit Exceeded",
          ...
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: async () => {
              try {
                ...
                await AsyncStorage.setItem("last_clear_cache_time", Date.now().toString());

                Alert.alert(
                  "Success", 
                  "Cache cleared. App will now restart.", 
                  [
                    {
                      text: "OK", 
                      onPress: async () => {
                        try {
                          ...
                          console.log("Could not reload async. User needs to restart manually.", e);
                        }
                      }
                    }
                  ]
