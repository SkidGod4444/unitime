...
export default function AttendanceListener() {
  ...
  const checkActiveSessions = React.useCallback(async () => {
    ...
    if (currentPath.includes("tap-to-mark") || currentPath.includes("attendance-session-form")) {
      return;
    }
        ...
        if (json.success && json.data?.activeSessions?.length > 0) {
          ...
          if (!markedIds.includes(session.id)) {
            ...
            router.push({
              ...
              params: { sessionId: session.id, courseName: session.course?.name || "Class" },
            } as any);
  ...
  useEffect(() => {
    ...
    const notificationListener = Notifications.addNotificationReceivedListener((_notif) => {
