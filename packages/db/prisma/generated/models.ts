
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/Courses.ts'
export type * from './models/Notification.ts'
export type * from './models/HistoryLog.ts'
export type * from './models/User.ts'
export type * from './models/UserCourse.ts'
export type * from './models/UserTimetable.ts'
export type * from './models/Timetable.ts'
export type * from './models/AttendanceQRSession.ts'
export type * from './models/AttendanceLogs.ts'
export type * from './models/AttendanceSummary.ts'
export type * from './models/StudentProfile.ts'
export type * from './models/Organization.ts'
export type * from './models/Alarm.ts'
export type * from './models/LabGroup.ts'
export type * from './models/StudentLabGroup.ts'
export type * from './commonInputTypes.ts'