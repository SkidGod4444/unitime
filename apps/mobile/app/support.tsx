      ...
      >
        ...
        <View className="px-4 pt-4">
          <View className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-4">
            <Text className="text-base font-bold text-gray-900 dark:text-zinc-100 mb-1">
              Raise a Ticket
            </Text>
            <Text className="text-xs text-gray-500 dark:text-zinc-400 mb-3">
            ...
            >
              <Text
                className={`font-bold ${canSubmit ? "text-white" : "text-gray-500"}`}
              >
                Submit
              </Text>
            </TouchableOpacity>
        ...
        <View className="px-4 mt-6">
          <Text className="text-base font-bold text-gray-800 dark:text-zinc-100 mb-3">
            My Tickets
          </Text>
          {myTickets.length === 0 ? (
            <View className="bg-white dark:bg-zinc-900 rounded-2xl border border-dashed border-gray-200 dark:border-zinc-800 p-6 items-center">
              ...
              <Text className="text-gray-500 dark:text-zinc-400 mt-2 text-sm">
                No tickets yet
              </Text>
            </View>
          ) : (
                ...
                return (
                  <View className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-4 mb-3">
                    <View className="flex-row items-center justify-between">
                      <Text
                        className="font-bold text-gray-900 dark:text-zinc-100 pr-2"
                        numberOfLines={1}
                      >
                        {t.title}
                      </Text>
                    ...
                    <Text
                      className="text-sm text-gray-600 dark:text-zinc-300 mt-1"
                      numberOfLines={3}
                    >
                      {t.description}
                    </Text>
                    {t.resolutionNote ? (
                      <View className="mt-2 bg-green-50 dark:bg-green-900/20 p-2 rounded-xl border border-green-100 dark:border-green-900/30">
                        ...
                        <Text className="text-sm text-green-700 dark:text-green-200 mt-0.5">
                          {t.resolutionNote}
                        </Text>
                      </View>
                    ) : null}
